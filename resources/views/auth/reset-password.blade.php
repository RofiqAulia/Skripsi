<x-guest-layout>
    ...
    <div class="mb-4 text-sm text-gray-600 text-center" style="margin-bottom: 25px;">
        Silakan masukkan kode OTP yang telah dikirimkan ke email Anda, beserta password baru yang Anda inginkan.
    </div>
    ...
    <form method="POST" action="{{ route('password.store') }}">
        ...
        <div class="input-group">
            <input type="email" name="email" value="{{ old('email', $email ?? '') }}" required readonly placeholder="Email">
            @error('email')
        ...
        <!-- OTP -->
        <div class="input-group">
            <input type="text" name="otp" required autofocus placeholder="Kode OTP (6 digit)" maxlength="6">
            @error('otp')
                <span class="text-error">{{ $message }}</span>
            @enderror
        </div>
