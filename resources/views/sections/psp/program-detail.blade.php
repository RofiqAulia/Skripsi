...
<section class="detail-hero">
    <div class="container-lg">
        ...
        <div class="detail-hero-badge">
            @if($program->country)
                <span class="hero-badge-item badge-country">{{ $program->country }}</span>
            @endif
            @if($program->funding_type)
                <span class="hero-badge-item badge-funding">{{ $program->funding_type }}</span>
            @endif
            @php
        ...
        <div class="meta-row">
            @if($program->open_date)
                <div class="meta-item">
                    ...
                    <strong>{{ $program->open_date->format('d M Y') }}</strong>
                </div>
            @endif
            @if($program->deadline)
                <div class="meta-item">
                    <small>Deadline</small>
                    <strong>{{ $program->deadline->format('d M Y') }}</strong>
                </div>
            @endif
            @if($program)
                <div class="meta-item">
    ...
    <div class="container-lg">
        ...
        <div class="detail-card">
            ...
            <div class="req-grid">
                ...
                @if($program->nationality)
                    <div class="req-item">
                        ...
                        <span>{{ $program->nationality }}</span>
                    </div>
                @endif
                @if($program->other_language)
                    <div class="req-item">
                        <label>Other Language</label>
                        <span>{{ $program->other_language }}</span>
                    </div>
                @endif
                @if($program->standardized_test)
                    <div class="req-item">
                        <label>Standardized Test</label>
                        <span>{{ $program->standardized_test }}</span>
                    </div>
                @endif
            </div>

            {{-- English Tests --}}
            @if($program->english_test && count($program->english_test) > 0)
                <div class="mt-4">
                    <label style="font-size:13px; color:#6b7280; font-weight:600;">English Proficiency Tests</label>
                    <div class="tag-list">
                        @foreach($program->english_test as $test)
                            <div class="tag-item">
                                {{ $test['test_name'] ?? '-' }}
            ...
            @if($program->other)
                <div class="mt-4">
                    ...
                    <p style="color:#374151; margin-top:6px;">{{ $program->other }}</p>
                </div>
            @endif
        </div>

        {{-- === BENEFITS === --}}
        @if($program->benefit && count($program->benefit) > 0)
        <div class="detail-card">
            <div class="section-title">Benefits</div>
            <ul class="benefit-list">
                @foreach($program->benefit as $b)
        ...
        <div class="detail-card">
            ...
            <div class="req-grid">
                ...
                <div class="req-item">
                    <label>Website</label>
                    <span>
                        <a href="{{ $program->website }}" target="_blank" style="color:#8b0000;">
                            Visit →
                        </a>
                    </span>
                </div>
        ...
        <div class="cta-bar">
            <div>
                ...
                <p>Click "Choose This Scholarship" to select it and proceed with your study plan.</p>
            </div>
            <a href="{{ route('psp') }}#submit-study-plan" class="btn-pilih" onclick="localStorage.setItem('psp_from_detail', '{{ $program->id }}')">
